...
def analyze():
    reviews = json.loads(open("../reviews.json", "r").read())["data"]
    """
...
def infer():
    ...
    high_probability_fakes = list(filter(lambda g: len(g) > 1 and len(re.findall(r'\w+', g[0]["text"])) > 9, groups))
    f = open("low_probability_fakes.json", "w")
    f.write(json.dumps(clean_review_groups(low_probability_fakes), indent=2))
    f.close()
    f = open("high_probability_fakes.json", "w")
